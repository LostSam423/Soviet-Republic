...
int main()
{
    ...
    Shader *shdr = new Shader("shaders/shad1.vs","shaders/shad.fs");
    Texture *txt1 = new Texture("Images/glass1.jpg");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        GLuint uniformloc = glGetUniformLocation(shdr->returnID(),"transform" );
        glUniformMatrix4fv(uniformloc, 1, GL_FALSE, glm::value_ptr(trans));

        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
        trans = glm::mat4(1.0f);
        trans =glm::translate(trans, glm::vec3(-0.5f,0.5f,0.0f));
        float lol = sin(glfwGetTime());
        trans = scale(trans,glm::vec3(lol, lol, lol));
        glUniformMatrix4fv(uniformloc, 1, GL_FALSE, glm::value_ptr(trans));
        glBindVertexArray( VAO );
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
        ...
        trans = glm::mat4(1.0f);
        trans = glm::translate(trans,glm::vec3(0.5f, 0.5f, 0.0f));
        trans = glm::scale(trans, glm::vec3(lol, 1.0f, 1.0f));
        trans = glm::rotate(trans, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));
        glUniformMatrix4fv(uniformloc, 1, GL_FALSE, glm::value_ptr(trans));
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
        ...
        trans = glm::mat4(1.0f);
        trans = glm::translate(trans,glm::vec3(-0.5f, -0.5f, 0.0f));
        trans = glm::scale(trans, glm::vec3(1.0f, lol, 1.0f));
        trans = glm::rotate(trans, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f));
        glUniformMatrix4fv(uniformloc, 1, GL_FALSE, glm::value_ptr(trans));
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
